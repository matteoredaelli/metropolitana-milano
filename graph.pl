edge('Affori FN', 'Affori centro', m3).
edge('Affori centro', 'Dergano', m3).
edge('Affori', 'Bovisa - Politecnico', s2).
edge('Affori', 'Bovisa', s4).
edge('Airuno', 'Olgiate', s8).
edge('Albate Camerlata', 'Cucciago', s11).
edge('Albizzate', 'Cavaria Oggiona Jerago', s5).
edge('Amendola Fiera', 'Lotto', m1).
edge('Arcore', 'Monza', s8).
edge('Argonne', 'Forlanini', m4).
edge('Assago Milanofiori Forum', 'Assago Milanofiori Nord', m2).
edge('Assago Milanofiori Nord', 'Famagosta', m2).
edge('Bande Nere', 'Primaticcio', m1).
edge('Besana', 'Villa Raverio', s7).
edge('Biassono-Lesmo', 'Buttafava', s7).
edge('Bicocca', 'Cà Granda', m5).
edge('Bignami', 'Ponale', m5).
edge('Bollate Centro', 'Novate Milanese', s1).
edge('Bollate Centro', 'Novate Milanese', s3).
edge('Bollate Nord', 'Bollate Centro', s1).
edge('Bollate Nord', 'Bollate Centro', s3).
edge('Bonola', 'San Leonardo', m1).
edge('Borgolombardo', 'S. Giuliano Milanese', s1).
edge('Bovisa - Politecnico', 'Lancetti', s2).
edge('Bovisa', 'Domodossola', s3).
edge('Bovisa', 'Domodossola', s4).
edge('Bovisa', 'Lancetti', s1).
edge('Bovisa', 'Lancetti', s13).
edge('Bovisio Masciago - Mombello', 'Varedo', s2).
edge('Bovisio Masciago', 'Varedo', s4).
edge('Brenta', 'Corvetto', m3).
edge('Bruzzano', 'Affori', s2).
edge('Bruzzano', 'Affori', s4).
edge('Buonarroti', 'Amendola Fiera', m1).
edge('Bussero', 'Villa Pompea', m2).
edge('Busto Arsizio', 'Legnano', s5).
edge('Buttafava', 'Villasanta', s7).
edge('Cabiate', 'Meda', s2).
edge('Cadorna', 'Conciliazione', m1).
edge('Cadorna', 'Lanza', m2).
edge('Caiazzo', 'Loreto', m2).
edge('Cairoli', 'Cadorna', m1).
edge('Calolziocorte', 'Airuno', s8).
edge('Camnago Lentate', 'Seregno', s11).
edge('Camnago Lentate', 'Seveso', s4).
edge('Canegrate', 'Parabiago', s5).
edge('Cantù Cermenate', 'Carimate', s11).
edge('Carate-Calò', 'Triuggio-Ponte Albiate', s7).
edge('Carimate', 'Camnago Lentate', s11).
edge('Carnate-Usmate', 'Arcore', s8).
edge('Caronno Pertusella', 'Cesate', s1).
edge('Caronno Pertusella', 'Cesate', s3).
edge('Cascina Antonietta', 'Gessate', m2).
edge('Cascina Burrona', 'Cernusco', m2).
edge('Cascina Gobba', 'Cologno Sud', m2).
edge('Cascina Gobba', 'Vimodrone', m2).
edge('Cassago-Nibionno-Bulciago', 'Renate-Veduggio', s7).
edge('Cassano d\'Adda', 'Treviglio', s5).
edge('Cassano d\'Adda', 'Treviglio', s6).
edge('Cassina de\' Pecchi', 'Bussero', m2).
edge('Castronno', 'Albizzate', s5).
edge('Cavaria Oggiona Jerago', 'Gallarate', s5).
edge('Cenisio', 'Gerusalemme', m5).
edge('Centrale', 'Repubblica', m3).
edge('Ceriano Laghetto-Groane', 'Cesano Maderno-Groane', s9).
edge('Ceriano Laghetto-Solaro', 'Ceriano Laghetto-Groane', s9).
edge('Cernusco', 'Villa Fiorita', m2).
edge('Cernusco-Merate', 'Osnago', s8).
edge('Certosa di Pavia', 'Pavia', s13).
edge('Certosa', 'Rho Fiera', s11).
edge('Certosa', 'Villapizzone', s14).
edge('Certosa', 'Villapizzone', s5).
edge('Certosa', 'Villapizzone', s6).
edge('Cesano Boscone', 'Trezzano sul Naviglio', s9).
edge('Cesano Maderno', 'Bovisio Masciago - Mombello', s2).
edge('Cesano Maderno', 'Bovisio Masciago', s4).
edge('Cesano Maderno', 'Seveso Baruccana', s9).
edge('Cesano Maderno-Groane', 'Cesano Maderno', s9).
edge('Cesate', 'Garbagnate Milanese', s1).
edge('Cesate', 'Garbagnate Milanese', s3).
edge('Chiasso', 'Como San Giovanni', s11).
edge('Cimiano', 'Crescenzago', m2).
edge('Civate', 'Sala al Barro-Galbiate', s7).
edge('Cologno Centro', 'Cologno Nord', m2).
edge('Cologno Sud', 'Cologno Centro', m2).
edge('Comasina', 'Affori FN', m3).
edge('Como San Giovanni', 'Albate Camerlata', s11).
edge('Conciliazione', 'Pagano', m1).
edge('Corbetta S. Stefano', 'Vittuone', s6).
edge('Cordusio', 'Cairoli', m1).
edge('Cormano-Cusano', 'Bruzzano', s2).
edge('Cormano-Cusano', 'Bruzzano', s4).
edge('Corsico', 'Cesano Boscone', s9).
edge('Corvetto', 'Porto di Mare', m3).
edge('Costa Masnaga', 'Cassago-Nibionno-Bulciago', s7).
edge('Crescenzago', 'Cascina Gobba', m2).
edge('Crocetta', 'Porta Romana', m3).
edge('Cucciago', 'Cantù Cermenate', s11).
edge('Cà Granda', 'Istria', m5).
edge('Dateo', 'Porta Vittoria', s1).
edge('Dateo', 'Porta Vittoria', s13).
edge('Dateo', 'Porta Vittoria', s14).
edge('Dateo', 'Porta Vittoria', s2).
edge('Dateo', 'Porta Vittoria', s5).
edge('Dateo', 'Porta Vittoria', s6).
edge('Dateo', 'Susa', m4).
edge('De Angeli', 'Gambara', m1).
edge('Dergano', 'Maciachini', m3).
edge('Desio', 'Lissone', s11).
edge('Desio', 'Lissone', s9).
edge('Domodossola FNM', 'Tre torri', m5).
edge('Domodossola', 'Cadorna', s3).
edge('Domodossola', 'Cadorna', s4).
edge('Duomo', 'Cordusio', m1).
edge('Duomo', 'Missori', m3).
edge('Famagosta', 'Romolo', m2).
edge('Forlanini', 'Lodi T.I.B.B. (P.ta Romana FS)', s9).
edge('Forlanini', 'Repetti', m4).
edge('Forlanini', 'Segrate', s5).
edge('Forlanini', 'Segrate', s6).
edge('Gaggiano', 'Albairate - Vermezzo', s9).
edge('Gallarate', 'Busto Arsizio', s5).
edge('Gambara', 'Bande Nere', m1).
edge('Garbagnate - Groane', 'Bollate Nord', s1).
edge('Garbagnate Milanese', 'Garbagnate - Groane', s1).
edge('Garbagnate Milanese', 'Garbagnate Parco delle Groane', s3).
edge('Garbagnate Parco delle Groane', 'Bollate Nord', s3).
edge('Gazzada', 'Castronno', s5).
edge('Gerusalemme', 'Domodossola FNM', m5).
edge('Gioia', 'Milano Centrale', m2).
edge('Gorgonzola', 'Cascina Antonietta', m2).
edge('Gorla', 'Turro', m1).
edge('Greco Pirelli', 'Lambrate', s9).
edge('Greco Pirelli', 'Porta Garibaldi', s11).
edge('Greco Pirelli', 'Porta Garibaldi', s7).
edge('Greco Pirelli', 'Porta Garibaldi', s8).
edge('Inganni', 'Bisceglie', m1).
edge('Isola', 'Porta Garibaldi', m5).
edge('Istria', 'Marche', m5).
edge('Lambrate', 'Forlanini', s9).
edge('Lambrate', 'Udine', m2).
edge('Lampugnano', 'Uruguay', m1).
edge('Lancetti', 'Porta Garibaldi', s1).
edge('Lancetti', 'Porta Garibaldi', s13).
edge('Lancetti', 'Porta Garibaldi', s14).
edge('Lancetti', 'Porta Garibaldi', s2).
edge('Lancetti', 'Porta Garibaldi', s5).
edge('Lancetti', 'Porta Garibaldi', s6).
edge('Lanza', 'Moscova', m2).
edge('Lecco Maggianico', 'Calolziocorte', s8).
edge('Lecco', 'Lecco Maggianico', s8).
edge('Lecco', 'Valmadrera', s7).
edge('Legnano', 'Canegrate', s5).
edge('Lima', 'Venezia', m1).
edge('Lissone', 'Monza', s11).
edge('Lissone', 'Monza', s9).
edge('Locate Triulzi', 'Pieve Emanuele', s13).
edge('Lodi T.I.B.B. (P.ta Romana FS)', 'Brenta', m3).
edge('Lodi T.I.B.B. (P.ta Romana FS)', 'Romolo', s9).
edge('Loreto', 'Lima', m1).
edge('Loreto', 'Piola', m2).
edge('Lotto', 'QT8', m1).
edge('Lotto', 'Segesta', m5).
edge('Macherio-Canonica', 'Biassono-Lesmo', s7).
edge('Maciachini', 'Zara', m3).
edge('Magenta', 'Corbetta S. Stefano', s6).
edge('Marche', 'Zara', m5).
edge('Mariano Comense', 'Cabiate', s2).
edge('Meda', 'Seveso', s2).
edge('Melegnano', 'S. Zenone al Lambro', s1).
edge('Melzo', 'Pozzuolo Martesana', s5).
edge('Melzo', 'Pozzuolo Martesana', s6).
edge('Milano Centrale', 'Caiazzo', m2).
edge('Missori', 'Crocetta', m3).
edge('Molino Dorino', 'Pero', m1).
edge('Molteno', 'Costa Masnaga', s7).
edge('Montenapoleone', 'Duomo', m3).
edge('Monumentale', 'Cenisio', m5).
edge('Monza Sobborghi', 'Monza', s7).
edge('Monza', 'Sesto FS (I Maggio)', s11).
edge('Monza', 'Sesto FS (I Maggio)', s7).
edge('Monza', 'Sesto FS (I Maggio)', s8).
edge('Monza', 'Sesto FS (I Maggio)', s9).
edge('Moscova', 'Porta Garibaldi', m2).
edge('Novara', 'Trecate', s6).
edge('Novate Milanese', 'Quarto Oggiaro', s1).
edge('Novate Milanese', 'Quarto Oggiaro', s3).
edge('Oggiono', 'Molteno', s7).
edge('Olgiate', 'Cernusco-Merate', s8).
edge('Osnago', 'Carnate-Usmate', s8).
edge('Paderno Dugnano', 'Cormano-Cusano', s2).
edge('Paderno Dugnano', 'Cormano-Cusano', s4).
edge('Pagano', 'Buonarroti', m1).
edge('Pagano', 'Wagner', m1).
edge('Palazzolo Milanese', 'Paderno Dugnano', s2).
edge('Palazzolo Milanese', 'Paderno Dugnano', s4).
edge('Palestro', 'San Babila', m1).
edge('Parabiago', 'Vanzago', s5).
edge('Pasteur', 'Loreto', m1).
edge('Pero', 'Rho Fiera', m1).
edge('Piazza Abbiategrasso', 'Famagosta', m2).
edge('Pieve Emanuele', 'Villamaggiore', s13).
edge('Piola', 'Lambrate', m2).
edge('Pioltello', 'Vignate', s5).
edge('Pioltello', 'Vignate', s6).
edge('Ponale', 'Bicocca', m5).
edge('Porta Garibaldi', 'Gioia', m2).
edge('Porta Garibaldi', 'Monumentale', m5).
edge('Porta Garibaldi', 'Repubblica', s1).
edge('Porta Garibaldi', 'Repubblica', s13).
edge('Porta Garibaldi', 'Repubblica', s14).
edge('Porta Garibaldi', 'Repubblica', s2).
edge('Porta Garibaldi', 'Repubblica', s5).
edge('Porta Garibaldi', 'Repubblica', s6).
edge('Porta Garibaldi', 'Villapizzone', s11).
edge('Porta Genova', 'Sant\'Agostino', m2).
edge('Porta Romana', 'Lodi T.I.B.B. (P.ta Romana FS)', m3).
edge('Porta Venezia', 'Dateo', s1).
edge('Porta Venezia', 'Dateo', s13).
edge('Porta Venezia', 'Dateo', s14).
edge('Porta Venezia', 'Dateo', s2).
edge('Porta Venezia', 'Dateo', s5).
edge('Porta Venezia', 'Dateo', s6).
edge('Porta Vittoria', 'Forlanini', s5).
edge('Porta Vittoria', 'Forlanini', s6).
edge('Porta Vittoria', 'Rogoredo', s1).
edge('Porta Vittoria', 'Rogoredo', s13).
edge('Porta Vittoria', 'Rogoredo', s14).
edge('Porta Vittoria', 'Rogoredo', s2).
edge('Portello', 'Lotto', m5).
edge('Porto di Mare', 'Rogoredo', m3).
edge('Pozzuolo Martesana', 'Trecella', s5).
edge('Pozzuolo Martesana', 'Trecella', s6).
edge('Precotto', 'Gorla', m1).
edge('Pregnana Milanese', 'Rho', s6).
edge('Primaticcio', 'Inganni', m1).
edge('QT8', 'Lampugnano', m1).
edge('Quarto Oggiaro', 'Bovisa', s1).
edge('Quarto Oggiaro', 'Bovisa', s3).
edge('Renate-Veduggio', 'Besana', s7).
edge('Repetti', 'Linate', m4).
edge('Repubblica', 'Porta Venezia', s1).
edge('Repubblica', 'Porta Venezia', s13).
edge('Repubblica', 'Porta Venezia', s14).
edge('Repubblica', 'Porta Venezia', s2).
edge('Repubblica', 'Porta Venezia', s5).
edge('Repubblica', 'Porta Venezia', s6).
edge('Repubblica', 'Turati', m3).
edge('Rho Fiera', 'Certosa', s14).
edge('Rho Fiera', 'Rho', s11).
edge('Rho FieraMilano', 'Certosa', s5).
edge('Rho FieraMilano', 'Certosa', s6).
edge('Rho', 'Rho Fiera', s14).
edge('Rho', 'Rho FieraMilano', s5).
edge('Rho', 'Rho FieraMilano', s6).
edge('Rogoredo', 'Locate Triulzi', s13).
edge('Rogoredo', 'S. Donato Milanese', s1).
edge('Rogoredo', 'San Donato', m3).
edge('Romolo', 'Porta Genova', m2).
edge('Romolo', 'San Cristoforo', s9).
edge('Rovereto', 'Pasteur', m1).
edge('S. Donato Milanese', 'Borgolombardo', s1).
edge('S. Giuliano Milanese', 'Melegnano', s1).
edge('S. Zenone al Lambro', 'Tavazzano', s1).
edge('Sala al Barro-Galbiate', 'Oggiono', s7).
edge('San Babila', 'Duomo', m1).
edge('San Babila', 'Tricolore', m4).
edge('San Cristoforo', 'Corsico', s9).
edge('San Leonardo', 'Molino Dorino', m1).
edge('San Siro Ippodromo', 'San Siro Stadio', m5).
edge('Sant\'Agostino', 'Sant\'Ambrogio', m2).
edge('Sant\'Ambrogio', 'Cadorna', m2).
edge('Saronno Sud', 'Caronno Pertusella', s1).
edge('Saronno Sud', 'Caronno Pertusella', s3).
edge('Saronno Sud', 'Ceriano Laghetto-Solaro', s9).
edge('Saronno', 'Saronno Sud', s1).
edge('Saronno', 'Saronno Sud', s3).
edge('Saronno', 'Saronno Sud', s9).
edge('Segesta', 'San Siro Ippodromo', m5).
edge('Segrate', 'Pioltello', s5).
edge('Segrate', 'Pioltello', s6).
edge('Seregno', 'Desio', s11).
edge('Seregno', 'Desio', s9).
edge('Sesto FS (I Maggio)', 'Greco Pirelli', s11).
edge('Sesto FS (I Maggio)', 'Greco Pirelli', s7).
edge('Sesto FS (I Maggio)', 'Greco Pirelli', s8).
edge('Sesto FS (I Maggio)', 'Greco Pirelli', s9).
edge('Sesto FS (I Maggio)', 'Sesto Rondò', m1).
edge('Sesto Marelli', 'Villa S. Giovanni', m1).
edge('Sesto Rondò', 'Sesto Marelli', m1).
edge('Seveso Baruccana', 'Seregno', s9).
edge('Seveso', 'Cesano Maderno', s2).
edge('Seveso', 'Cesano Maderno', s4).
edge('Sondrio', 'Centrale', m3).
edge('Susa', 'Argonne', m4).
edge('Tavazzano', 'Lodi', s1).
edge('Tre torri', 'Portello', m5).
edge('Trecate', 'Magenta', s6).
edge('Trecella', 'Cassano d\'Adda', s5).
edge('Trecella', 'Cassano d\'Adda', s6).
edge('Trezzano sul Naviglio', 'Gaggiano', s9).
edge('Tricolore', 'Dateo', m4).
edge('Triuggio-Ponte Albiate', 'Macherio-Canonica', s7).
edge('Turati', 'Montenapoleone', m3).
edge('Turro', 'Rovereto', m1).
edge('Udine', 'Cimiano', m2).
edge('Uruguay', 'Bonola', m1).
edge('Valmadrera', 'Civate', s7).
edge('Vanzago', 'Rho', s5).
edge('Varedo', 'Palazzolo Milanese', s2).
edge('Varedo', 'Palazzolo Milanese', s4).
edge('Varese', 'Gazzada', s5).
edge('Venezia', 'Palestro', m1).
edge('Vignate', 'Melzo', s5).
edge('Vignate', 'Melzo', s6).
edge('Villa Fiorita', 'Cassina de\' Pecchi', m2).
edge('Villa Pompea', 'Gorgonzola', m2).
edge('Villa Raverio', 'Carate-Calò', s7).
edge('Villa S. Giovanni', 'Precotto', m1).
edge('Villamaggiore', 'Certosa di Pavia', s13).
edge('Villapizzone', 'Certosa', s11).
edge('Villapizzone', 'Lancetti', s14).
edge('Villapizzone', 'Lancetti', s5).
edge('Villapizzone', 'Lancetti', s6).
edge('Villasanta', 'Monza Sobborghi', s7).
edge('Vimodrone', 'Cascina Burrona', m2).
edge('Vittuone', 'Pregnana Milanese', s6).
edge('Wagner', 'De Angeli', m1).
edge('Zara', 'Isola', m5).
edge('Zara', 'Sondrio', m3).
station('Affori FN', m3).
station('Affori centro', m3).
station('Affori', s2).
station('Affori', s4).
station('Airuno', s8).
station('Albairate - Vermezzo', s9).
station('Albate Camerlata', s11).
station('Albizzate', s5).
station('Amendola Fiera', m1).
station('Arcore', s8).
station('Argonne', m4).
station('Assago Milanofiori Forum', m2).
station('Assago Milanofiori Nord', m2).
station('Bande Nere', m1).
station('Besana', s7).
station('Biassono-Lesmo', s7).
station('Bicocca', m5).
station('Bignami', m5).
station('Bisceglie', m1).
station('Bollate Centro', s1).
station('Bollate Centro', s3).
station('Bollate Nord', s1).
station('Bollate Nord', s3).
station('Bonola', m1).
station('Borgolombardo', s1).
station('Bovisa - Politecnico', s2).
station('Bovisa', s1).
station('Bovisa', s13).
station('Bovisa', s3).
station('Bovisa', s4).
station('Bovisio Masciago - Mombello', s2).
station('Bovisio Masciago', s4).
station('Brenta', m3).
station('Bruzzano', s2).
station('Bruzzano', s4).
station('Buonarroti', m1).
station('Bussero', m2).
station('Busto Arsizio', s5).
station('Buttafava', s7).
station('Cabiate', s2).
station('Cadorna', m1).
station('Cadorna', m2).
station('Cadorna', s3).
station('Cadorna', s4).
station('Caiazzo', m2).
station('Cairoli', m1).
station('Calolziocorte', s8).
station('Camnago Lentate', s11).
station('Camnago Lentate', s4).
station('Canegrate', s5).
station('Cantù Cermenate', s11).
station('Carate-Calò', s7).
station('Carimate', s11).
station('Carnate-Usmate', s8).
station('Caronno Pertusella', s1).
station('Caronno Pertusella', s3).
station('Cascina Antonietta', m2).
station('Cascina Burrona', m2).
station('Cascina Gobba', m2).
station('Cascina Gobba', m2).
station('Cassago-Nibionno-Bulciago', s7).
station('Cassano d\'Adda', s5).
station('Cassano d\'Adda', s6).
station('Cassina de\' Pecchi', m2).
station('Castronno', s5).
station('Cavaria Oggiona Jerago', s5).
station('Cenisio', m5).
station('Centrale', m3).
station('Ceriano Laghetto-Groane', s9).
station('Ceriano Laghetto-Solaro', s9).
station('Cernusco', m2).
station('Cernusco-Merate', s8).
station('Certosa di Pavia', s13).
station('Certosa', s11).
station('Certosa', s14).
station('Certosa', s5).
station('Certosa', s6).
station('Cesano Boscone', s9).
station('Cesano Maderno', s2).
station('Cesano Maderno', s4).
station('Cesano Maderno', s9).
station('Cesano Maderno-Groane', s9).
station('Cesate', s1).
station('Cesate', s3).
station('Chiasso', s11).
station('Cimiano', m2).
station('Civate', s7).
station('Cologno Centro', m2).
station('Cologno Nord', m2).
station('Cologno Sud', m2).
station('Comasina', m3).
station('Como San Giovanni', s11).
station('Conciliazione', m1).
station('Corbetta S. Stefano', s6).
station('Cordusio', m1).
station('Cormano-Cusano', s2).
station('Cormano-Cusano', s4).
station('Corsico', s9).
station('Corvetto', m3).
station('Costa Masnaga', s7).
station('Crescenzago', m2).
station('Crocetta', m3).
station('Cucciago', s11).
station('Cà Granda', m5).
station('Dateo', m4).
station('Dateo', s1).
station('Dateo', s13).
station('Dateo', s14).
station('Dateo', s2).
station('Dateo', s5).
station('Dateo', s6).
station('De Angeli', m1).
station('Dergano', m3).
station('Desio', s11).
station('Desio', s9).
station('Domodossola FNM', m5).
station('Domodossola', s3).
station('Domodossola', s4).
station('Duomo', m1).
station('Duomo', m3).
station('Famagosta', m2).
station('Famagosta', m2).
station('Forlanini', m4).
station('Forlanini', s5).
station('Forlanini', s6).
station('Forlanini', s9).
station('Gaggiano', s9).
station('Gallarate', s5).
station('Gambara', m1).
station('Garbagnate - Groane', s1).
station('Garbagnate Milanese', s1).
station('Garbagnate Milanese', s3).
station('Garbagnate Parco delle Groane', s3).
station('Gazzada', s5).
station('Gerusalemme', m5).
station('Gessate', m2).
station('Gioia', m2).
station('Gorgonzola', m2).
station('Gorla', m1).
station('Greco Pirelli', s11).
station('Greco Pirelli', s7).
station('Greco Pirelli', s8).
station('Greco Pirelli', s9).
station('Inganni', m1).
station('Isola', m5).
station('Istria', m5).
station('Lambrate', m2).
station('Lambrate', s9).
station('Lampugnano', m1).
station('Lancetti', s1).
station('Lancetti', s13).
station('Lancetti', s14).
station('Lancetti', s2).
station('Lancetti', s5).
station('Lancetti', s6).
station('Lanza', m2).
station('Lecco Maggianico', s8).
station('Lecco', s7).
station('Lecco', s8).
station('Legnano', s5).
station('Lima', m1).
station('Linate', m4).
station('Lissone', s11).
station('Lissone', s9).
station('Locate Triulzi', s13).
station('Lodi T.I.B.B. (P.ta Romana FS)', m3).
station('Lodi T.I.B.B. (P.ta Romana FS)', s9).
station('Lodi', s1).
station('Loreto', m1).
station('Loreto', m2).
station('Lotto', m1).
station('Lotto', m5).
station('Macherio-Canonica', s7).
station('Maciachini', m3).
station('Magenta', s6).
station('Marche', m5).
station('Mariano Comense', s2).
station('Meda', s2).
station('Melegnano', s1).
station('Melzo', s5).
station('Melzo', s6).
station('Milano Centrale', m2).
station('Missori', m3).
station('Molino Dorino', m1).
station('Molteno', s7).
station('Montenapoleone', m3).
station('Monumentale', m5).
station('Monza Sobborghi', s7).
station('Monza', s11).
station('Monza', s7).
station('Monza', s8).
station('Monza', s9).
station('Moscova', m2).
station('Novara', s6).
station('Novate Milanese', s1).
station('Novate Milanese', s3).
station('Oggiono', s7).
station('Olgiate', s8).
station('Osnago', s8).
station('Paderno Dugnano', s2).
station('Paderno Dugnano', s4).
station('Pagano', m1).
station('Pagano', m1).
station('Palazzolo Milanese', s2).
station('Palazzolo Milanese', s4).
station('Palestro', m1).
station('Parabiago', s5).
station('Pasteur', m1).
station('Pavia', s13).
station('Pero', m1).
station('Piazza Abbiategrasso', m2).
station('Pieve Emanuele', s13).
station('Piola', m2).
station('Pioltello', s5).
station('Pioltello', s6).
station('Ponale', m5).
station('Porta Garibaldi', m2).
station('Porta Garibaldi', m5).
station('Porta Garibaldi', s1).
station('Porta Garibaldi', s11).
station('Porta Garibaldi', s13).
station('Porta Garibaldi', s14).
station('Porta Garibaldi', s2).
station('Porta Garibaldi', s5).
station('Porta Garibaldi', s6).
station('Porta Garibaldi', s7).
station('Porta Garibaldi', s8).
station('Porta Genova', m2).
station('Porta Romana', m3).
station('Porta Venezia', s1).
station('Porta Venezia', s13).
station('Porta Venezia', s14).
station('Porta Venezia', s2).
station('Porta Venezia', s5).
station('Porta Venezia', s6).
station('Porta Vittoria', s1).
station('Porta Vittoria', s13).
station('Porta Vittoria', s14).
station('Porta Vittoria', s2).
station('Porta Vittoria', s5).
station('Porta Vittoria', s6).
station('Portello', m5).
station('Porto di Mare', m3).
station('Pozzuolo Martesana', s5).
station('Pozzuolo Martesana', s6).
station('Precotto', m1).
station('Pregnana Milanese', s6).
station('Primaticcio', m1).
station('QT8', m1).
station('Quarto Oggiaro', s1).
station('Quarto Oggiaro', s3).
station('Renate-Veduggio', s7).
station('Repetti', m4).
station('Repubblica', m3).
station('Repubblica', s1).
station('Repubblica', s13).
station('Repubblica', s14).
station('Repubblica', s2).
station('Repubblica', s5).
station('Repubblica', s6).
station('Rho Fiera', m1).
station('Rho Fiera', s11).
station('Rho Fiera', s14).
station('Rho FieraMilano', s5).
station('Rho FieraMilano', s6).
station('Rho', s11).
station('Rho', s14).
station('Rho', s5).
station('Rho', s6).
station('Rogoredo', m3).
station('Rogoredo', s1).
station('Rogoredo', s13).
station('Rogoredo', s14).
station('Rogoredo', s2).
station('Romolo', m2).
station('Romolo', s9).
station('Rovereto', m1).
station('S. Donato Milanese', s1).
station('S. Giuliano Milanese', s1).
station('S. Zenone al Lambro', s1).
station('Sala al Barro-Galbiate', s7).
station('San Babila', m1).
station('San Babila', m4).
station('San Cristoforo', s9).
station('San Donato', m3).
station('San Leonardo', m1).
station('San Siro Ippodromo', m5).
station('San Siro Stadio', m5).
station('Sant\'Agostino', m2).
station('Sant\'Ambrogio', m2).
station('Saronno Sud', s1).
station('Saronno Sud', s3).
station('Saronno Sud', s9).
station('Saronno', s1).
station('Saronno', s3).
station('Saronno', s9).
station('Segesta', m5).
station('Segrate', s5).
station('Segrate', s6).
station('Seregno', s11).
station('Seregno', s9).
station('Sesto FS (I Maggio)', m1).
station('Sesto FS (I Maggio)', s11).
station('Sesto FS (I Maggio)', s7).
station('Sesto FS (I Maggio)', s8).
station('Sesto FS (I Maggio)', s9).
station('Sesto Marelli', m1).
station('Sesto Rondò', m1).
station('Seveso Baruccana', s9).
station('Seveso', s2).
station('Seveso', s4).
station('Sondrio', m3).
station('Susa', m4).
station('Tavazzano', s1).
station('Tre torri', m5).
station('Trecate', s6).
station('Trecella', s5).
station('Trecella', s6).
station('Treviglio', s5).
station('Treviglio', s6).
station('Trezzano sul Naviglio', s9).
station('Tricolore', m4).
station('Triuggio-Ponte Albiate', s7).
station('Turati', m3).
station('Turro', m1).
station('Udine', m2).
station('Uruguay', m1).
station('Valmadrera', s7).
station('Vanzago', s5).
station('Varedo', s2).
station('Varedo', s4).
station('Varese', s5).
station('Venezia', m1).
station('Vignate', s5).
station('Vignate', s6).
station('Villa Fiorita', m2).
station('Villa Pompea', m2).
station('Villa Raverio', s7).
station('Villa S. Giovanni', m1).
station('Villamaggiore', s13).
station('Villapizzone', s11).
station('Villapizzone', s14).
station('Villapizzone', s5).
station('Villapizzone', s6).
station('Villasanta', s7).
station('Vimodrone', m2).
station('Vittuone', s6).
station('Wagner', m1).
station('Zara', m3).
station('Zara', m5).
